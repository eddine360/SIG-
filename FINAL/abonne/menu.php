...
// abonements en cours 
	...
	for ($i=0; $i < 21; $i++) { 
		...
		$num=$plat["num_plat"];
		echo '<input type="hidden" name="plat'.$i.'" value="'.$num.'"></input>';
		//echo $num; 
		echo "</p></td>";
		// selection quantité 
		if ($bnef) {
...
//type='image'
			"
					...
					<input type='number' id='quantity' name='q".$i."' min='0' max='2' required>
		...
		echo "</div>";
		//récup num plat et menu 
		echo '<input type="hidden" name="var'.$i.'" value="'.$plat["num_menu"].'"></input>'; 
	}
	echo "</div>";
	//echo '<br>'.$plat["num_plat"]; 

				
echo '____________________________'.$plat["num_plat"]; 
}
